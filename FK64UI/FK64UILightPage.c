...
unsigned char LightTemp = 1;

int PageLinghtSetCallback(int t)
{
    if ((t == 1) || (t == -1))
    {
        LightTemp += t;
        if (LightTemp > 4) LightTemp = 1;
        if (LightTemp < 1) LightTemp = 4;
...
pageInfo* FK64LinghtPageShow(lv_obj_t* temp)
{
    // 创建一个 table
...
pageInfo* FK64LinghtPageSet(lv_obj_t* temp,pageInfo* homePage)
{
    // 创建一个 table
    lv_obj_t* T = lv_tabview_add_tab(temp, " ");
    lv_obj_set_style_border_color(T, lv_color_make(0xff, 0, 0), 0);
...
lv_obj_t* FK64UILinghtPageinit(lv_obj_t* temp)
{
    ...
    pageInfo* tempPage = FK64LinghtPageShow(tabview);
    FK64LinghtPageSet(tabview,tempPage);
