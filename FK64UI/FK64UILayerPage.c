...
int PageLayerSetCallback(int t)
{
    if ((t == 1) || (t == -1))
    {
        layerNumberChange(t);
...
pageInfo* FK64LayerPageShow(lv_obj_t* temp)
{
    // 创建一个 table
...
pageInfo* FK64LayerPageSet(lv_obj_t* temp,pageInfo* homePage)
{
    // 创建一个 table
    lv_obj_t* T = lv_tabview_add_tab(temp, " ");
    lv_obj_set_style_border_color(T, lv_color_make(0xff, 0, 0), 0);
...
lv_obj_t* FK64UILayerPageinit(lv_obj_t* temp)
{
    ...
    pageInfo* tempPage = FK64LayerPageShow(tabview);
    FK64LayerPageSet(tabview,tempPage);
