...
static void scroll_area_into_view(const lv_area_t * area, lv_obj_t * child, lv_point_t * scroll_value,
                                  lv_anim_enable_t anim_en)
{
    lv_obj_t * parent = lv_obj_get_parent(child);               //获取父对象
    if(!lv_obj_has_flag(parent, LV_OBJ_FLAG_SCROLLABLE)) return;//对象是不是禁止滚动

    lv_dir_t scroll_dir = lv_obj_get_scroll_dir(parent); //获取滚动方向
