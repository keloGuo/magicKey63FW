...
typedef enum {
    ...
    /** Input device events*/
    LV_EVENT_PRESSED,             /**< 1The object has been pressed*/
    LV_EVENT_PRESSING,            /**< 2The object is being pressed (called continuously while pressing)*/
    LV_EVENT_PRESS_LOST,          /**< 3The object is still being pressed but slid cursor/finger off of the object */
    LV_EVENT_SHORT_CLICKED,       /**< 4The object was pressed for a short period of time, then released it. Not called if scrolled.*/
    LV_EVENT_LONG_PRESSED,        /**< 5Object has been pressed for at least `long_press_time`.  Not called if scrolled.*/
    LV_EVENT_LONG_PRESSED_REPEAT, /**< 6Called after `long_press_time` in every `long_press_repeat_time` ms.  Not called if scrolled.*/
    LV_EVENT_CLICKED,             /**< 7Called on release if not scrolled (regardless to long press)*/
    LV_EVENT_RELEASED,            /**< 8Called in every cases when the object has been released*/
    LV_EVENT_SCROLL_BEGIN,        /**< 9Scrolling begins. The event parameter is a pointer to the animation of the scroll. Can be modified*/
    LV_EVENT_SCROLL_END,          /**< 10Scrolling ends*/
    LV_EVENT_SCROLL,              /**< 11Scrolling*/
    LV_EVENT_GESTURE,             /**< 12A gesture is detected. Get the gesture with `lv_indev_get_gesture_dir(lv_indev_get_act());` */
    LV_EVENT_KEY,                 /**< 13A key is sent to the object. Get the key with `lv_indev_get_key(lv_indev_get_act());`*/
    LV_EVENT_FOCUSED,             /**< 14The object is focused*/
    LV_EVENT_DEFOCUSED,           /**< 15The object is defocused*/
    LV_EVENT_LEAVE,               /**< 16The object is defocused but still selected*/
    LV_EVENT_HIT_TEST,            /**< 17Perform advanced hit-testing*/

    /** Drawing events*/
    LV_EVENT_COVER_CHECK,        /**< 18Check if the object fully covers an area. The event parameter is `lv_cover_check_info_t *`.*/
    LV_EVENT_REFR_EXT_DRAW_SIZE, /**< 19Get the required extra draw area around the object (e.g. for shadow). The event parameter is `lv_coord_t *` to store the size.*/
    LV_EVENT_DRAW_MAIN_BEGIN,    /**  20 < Starting the main drawing phase*/
    LV_EVENT_DRAW_MAIN,          /**< 21Perform the main drawing*/
    LV_EVENT_DRAW_MAIN_END,      /**< 22Finishing the main drawing phase*/
    LV_EVENT_DRAW_POST_BEGIN,    /**< 23Starting the post draw phase (when all children are drawn)*/
    LV_EVENT_DRAW_POST,          /**< 24Perform the post draw phase (when all children are drawn)*/
    LV_EVENT_DRAW_POST_END,      /**< 25 Finishing the post draw phase (when all children are drawn)*/
    LV_EVENT_DRAW_PART_BEGIN,    /**< 26 Starting to draw a part. The event parameter is `lv_obj_draw_dsc_t *`. */
    LV_EVENT_DRAW_PART_END,      /**< 27Finishing to draw a part. The event parameter is `lv_obj_draw_dsc_t *`. */

    /** Special events*/
    LV_EVENT_VALUE_CHANGED,       /**< 28The object's value has changed (i.e. slider moved)*/
    LV_EVENT_INSERT,              /**< 29A text is inserted to the object. The event data is `char *` being inserted.*/
    LV_EVENT_REFRESH,             /**< 30Notify the object to refresh something on it (for the user)*/
    LV_EVENT_READY,               /**< 31A process has finished*/
    LV_EVENT_CANCEL,              /**< 32A process has been cancelled */
