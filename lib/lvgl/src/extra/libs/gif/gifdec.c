...
static gd_GIF * gif_open(gd_GIF * gif_base)
{
    ...
    size_t gif_mem_size = 0;
#if LV_COLOR_DEPTH == 32
    gif_mem_size = sizeof(gd_GIF) + 5 * width * height;
#elif LV_COLOR_DEPTH == 16
    gif_mem_size = sizeof(gd_GIF) + 4 * width * height;
#elif LV_COLOR_DEPTH == 8 || LV_COLOR_DEPTH == 1
    gif_mem_size = sizeof(gd_GIF) + 3 * width * height;
#endif
    gif = scratchAcquire(gif_mem_size, "gif");
