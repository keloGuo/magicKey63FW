...
static int jsonOptionalInt(cJSON *root, const char *name, int min, int max, int *out)
{
    cJSON *item = cJSON_GetObjectItem(root, name);
    if(item == NULL || cJSON_IsNumber(item) == 0) return 0;
...
unsigned char webCodexStatus(struct mg_http_message *hm,struct mg_connection *c)
{
    ...
    int usagePercent = 0;
    unsigned char hasUsage = 0;
    if(jsonOptionalInt(tempJson, "usagePercent", 0, 100, &usagePercent) ||
       jsonOptionalInt(tempJson, "usedPercent", 0, 100, &usagePercent))
    {
        hasUsage = 1;
    }

    int tokenTotal = 0;
    jsonOptionalInt(tempJson, "tokenTotal", 0, 0x7fffffff, &tokenTotal);
    cJSON *usageDateJson = cJSON_GetObjectItem(tempJson, "usageDate");
    const char *usageDate = (usageDateJson != NULL && cJSON_IsString(usageDateJson)) ? usageDateJson->valuestring : NULL;
    magic63CodexUsageUpdate(hasUsage, (unsigned char)usagePercent, (unsigned int)tokenTotal, usageDate);
