...
lv_obj_t *LinghtShowObjF = NULL;
lv_obj_t *LinghtShowObjS = NULL;

unsigned char BackLightChange(signed char v);

unsigned char LightTemp = 5;
unsigned char getLight(void)
{
    return LightTemp;
}

unsigned char BackLightShowUpdte(unsigned char light)
{
    LightTemp = light;
    unsigned char temp[2] = {LightTemp + 0x30,'\0'};
    lv_label_set_text(LinghtShowObjF, (char*)(&temp));
    lv_label_set_text(LinghtShowObjS, (char*)(&temp));
    return 0;
}

int PageLinghtSetCallback(int t,int x,int k)
{    
    if ((t == 1) || (t == -1))
    {
        BackLightChange(t);
...
pageInfo* magic63LinghtPageShow(lv_obj_t* temp)
{
    // 创建一个 table
...
pageInfo* magic63LinghtPageSet(lv_obj_t* temp,pageInfo* homePage)
{
    // 创建一个 table
    lv_obj_t* T = lv_tabview_add_tab(temp, " ");
    lv_obj_set_style_border_color(T, lv_color_make(0xff, 0, 0), 0);
    ...
    pageRegister(homePage, temp, T, PageLinghtSetCallback);
    return NULL;
...
lv_obj_t* magic63UILinghtPageinit(lv_obj_t* temp)
{
    ...
    pageInfo* tempPage = magic63LinghtPageShow(tabview);
    magic63LinghtPageSet(tabview,tempPage);
